...
corrplot(cor_results, method = "circle", type = "upper", 
         tl.col = "black", tl.srt = 45, 
         addCoef.col = "black", number.cex = 0.7)
         


### Execute cor.test() to determine which columns to drop
         
numeric_vars <- data %>% select_if(is.numeric)

for (var in names(numeric_vars)) {
  if (var != "Exam_Score") {  # Skip the target variable itself
    test_result <- cor.test(numeric_vars[[var]], data$Exam_Score, method = "spearman")
    print("Spearman Correlation Test for:")
    print(var)
    print(test_result)
  }
}
